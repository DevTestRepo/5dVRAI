...
(() => {
  ...
  function addMsg(role, text) {
    ...
    // 1. Convert raw text URLs to clickable HTML links
    let processedText = linkify(text);

    // 2. Process the text through the PDF Viewer manager
    // This safely ignores the <a> tags we just created and only looks for ||PDF:...||
    processedText = PDFViewer.processText(processedText);

    // 3. Use innerHTML so the links and PDF spans render correctly
    div.innerHTML = processedText;
...
function linkify(inputText) {
    ...
    // Pattern 1: Match URLs starting with http:// or https://
    const replacePattern1 = /(\b(https?):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim;
    replacedText = inputText.replace(replacePattern1, '<a href="$1" target="_blank" class="chat-url-link" rel="noopener noreferrer">$1</a>');

    // Pattern 2: Match URLs starting with "www." (and automatically prepend https:// to the href)
    const replacePattern2 = /(^|[^\/])(\bwww\.[\S]+(\b|$))/gim;
    replacedText = replacedText.replace(replacePattern2, '$1<a href="https://$2" target="_blank" class="chat-url-link" rel="noopener noreferrer">$2</a>');
